#!/usr/bin/env python3
"""
Refresh a BANCONTACT Apple Pay workbook from CSVs and create a values-only copy.
...
CSV names accepted (first match wins):
  Generic:      Metrics.csv, Decline.csv, Usage.csv, Merchant.csv
  OR Bancontact: BANCONTACT_metrics*.csv, BANCONTACT_DECLINE*.csv,
                 BANCONTACT_USAGE*.csv, BANCONTACT_merchant*.csv

No Fraud / No circular references.
"""
...
DEFAULT_WIRED  = "applepay_rep_perf_BANCONTACT_WIRED_hidden.xlsx"
DEFAULT_READY  = "applepay_rep_perf_BANCONTACT_READY.xlsx"
...
def wire_visible_sheets(wb) -> None:
    ...
    if "Metrics" in wb.sheetnames and "Data_Metrics" in wb.sheetnames:
        ...
        # IMPORTANT: do NOT write any helper next to 'Monthly DPAN transaction count' in WIRED.
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="BANCONTACT Apple Pay workbook refresher")
